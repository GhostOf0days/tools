...
def convert_file(input_path: str, output_path: str, input_base_path: str):
    """
    ...
    :param input_base_path: The base directory of the input files
    """
    ...
    if input_extension == ".bin":
        ...
        # Create the output subdirectory structure if it doesn't exist
        output_subdir = os.path.join(output_path, os.path.dirname(relative_path))
        os.makedirs(output_subdir, exist_ok=True)
        
        # Generate the output file path
        output_file = os.path.join(output_subdir, os.path.splitext(os.path.basename(input_path))[0] + ".nfc")
        
        logging.info(f"Writing: {output_file}")
        with open(input_path, "rb") as file:
            contents = file.read()
            write_output(os.path.splitext(os.path.basename(input_path))[0], assemble_code(contents), output_subdir)

    elif input_extension == ".nfc":
        # Get the relative path of the input file from the input directory 
        relative_path = os.path.relpath(input_path, input_base_path)
        
        # Generate the output file path preserving the subdirectory structure
        output_file = os.path.join(output_path, relative_path) 
        
        # Create the output subdirectory structure if it doesn't exist
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
        
        save_ntag215_v2_with_pwd(input_path, output_file)
    else:
        logging.info(f"{input_path} doesn't seem like a relevant file, skipping")


def process(path: str, output_path: str, tree: bool, input_base_path: str):
    """
    ...
    :param output_path: The base directory to output to
    :param tree: Whether to preserve the directory structure
    :param input_base_path: The base directory of the input files
    """
    if os.path.isfile(path):
        convert_file(path, output_path, input_base_path)
    else:
        for root, dirs, files in os.walk(path):
            for filename in files:
                input_file_path = os.path.join(root, filename)
                logging.debug(f"Current file: {filename}; Current path: {input_file_path}")

                if tree:
                    relative_path = os.path.relpath(root, path)
                    new_output_path = os.path.join(output_path, relative_path)
                    os.makedirs(new_output_path, exist_ok=True)
                    convert_file(input_file_path, new_output_path, input_base_path)
                else:
                    convert_file(input_file_path, output_path, input_base_path)
...
def main():
    ...
    process(args.input_path, args.output_path, args.tree, args.input_path)
